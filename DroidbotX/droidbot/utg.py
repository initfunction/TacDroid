...
# from ted import extract_children
...
class UTG(object):
    ...
    def extract_children(tree):
        ...
        for child in children:
            child_node, child_count = UTG.extract_children(child)
            child_nodes.append(child_node)
            node_count += child_count

        return Node("root", child_nodes), node_count

    def calculate_treesimilarity(self, old_state, new_state):
        old_tree, old_count = UTG.extract_children(old_state.view_tree)
        current_tree, current_count = UTG.extract_children(new_state.view_tree)
        ted_distance = simple_distance(old_tree, current_tree)
        return ted_distance / max(old_count, current_count)

    @staticmethod
    def get_node_string(node):
        # 拼接节点信息
        node_info = f"{node['class']}_{node['bounds']}_{node['resource_id']}_{node['text']}"
        return node_info
    ...
    def calculate_nodesimilarity(self, old_state, new_state):
        tree_a = new_state.view_tree
        tree_b = old_state.view_tree
        similarities = []
        for node_a in tree_a['children']:
            node_a_info = UTG.get_node_string(node_a)
            matching_nodes = [node_b for node_b in tree_b['children'] if node_b['xpath'] == node_a['xpath']]
            for node_b in matching_nodes:
                node_b_info = UTG.get_node_string(node_b)
                similarity = UTG.calculate_similarity(node_a_info, node_b_info)
    ...
    def is_sameUI(self, old_state, new_state,threshold=0.7):
        UIsimilarity = self.calculate_treesimilarity(old_state,new_state) * self.calculate_nodesimilarity(old_state,new_state)
        if UIsimilarity > threshold:
            return True
        return False
    ...
    def add_transition(self, event, old_state, new_state):
        ...
        if self.is_sameUI(old_state,new_state):
