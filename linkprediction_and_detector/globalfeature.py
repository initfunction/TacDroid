...
class manifest_text_model(object):
    ...
    def testModel_Embedding(self):
        ...
        with torch.no_grad():
            for i, (names, labels) in enumerate(self.loader, 1):
                inputs, seq_lengths, target, inv_perm_index = self.make_tensors(names, labels)  
    ...
    class icon_dataset(Dataset):
        def __init__(self, icons, labels, ignore_none=False):
            ...
            for i in range(len(icons)):
                try:
                    temp_path = r'\Temp\icon.png'
                    shutil.copy(icons[i], temp_path)
                    image = cv2.imread(temp_path)
                    rgb_image_matrix = np.array(image)
...
if __name__ == '__main__':
    ...
    data = pd.read_csv(r'\base\part_manifest.csv').dropna()
    labels = list(data['类别'])
    nam = [i for i in data['名称'].fillna(value="")]
    pac = [i for i in data['包名'].fillna(value="")]
    cer = [i for i in data['签名'].fillna(value="")]
    X_string = [(nam[i]) + '; ' + pac[i] + '; ' + cer[i] for i in range(len(labels))]

    from sklearn.model_selection import train_test_split
    from sklearn.ensemble import RandomForestClassifier
    from sklearn.metrics import accuracy_score
    res = []
    train_texts, test_texts, train_labels, test_labels = train_test_split(X_string, labels, test_size=0.2)

    test_model = manifest_text_model(train_texts, train_labels)
    res, acc, qs = test_model.test_embedding('npc_model.pth')

    test_model = manifest_text_model(test_texts, test_labels)
    res, acc, ys = test_model.test_embedding('npc_model.pth')

    rf_classifier = RandomForestClassifier(n_estimators=100, random_state=42)
    rf_classifier.fit(qs, train_labels)     
    y_pred = rf_classifier.predict(ys)
