    ...
    def __init__(self, json_path):
        ...
        for js_transition in j.setdefault('transitions', []):
            ...
            if dst in self.activity:
                ...
                # self.activity[dst].setdefault('dst', []).append({'to': src, 'trigger': trigger})
            else:
                self.node.setdefault(dst, {}).setdefault('dst', {})[src] = {'trigger': trigger} 
                # self.node.setdefault(dst, {}).setdefault('dst', []).append({'to': src, 'trigger': trigger})
...
class new_dynamic_graph():
    ...
    def draw_graph_activity(self, pydotpath=None):
        ...
        for n in node:
            src = n
            dst_info = self.activity[n].setdefault('src', {})   # 获取当前节点作为起始点的所有边
            for d in dst_info:
                dst = d
    ...
    def get_text(self):
        '''获取动态图中每个活动对应的文本内容, 对于每个活动, 搜索全部状态中的字符串，并对明显重复内容进行筛选'''
        res_dict = {}
        similarity = 0.8
        for act in self.activity:
            res_text = []
